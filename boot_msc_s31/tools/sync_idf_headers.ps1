#===========================================================================
# sync_idf_headers.ps1 —— 把 boot_msc_s31 需要的 IDF 头文件"冻结"进工程
#
...
#   本项目因为手抄位号栽过两次，见 PSRAM.md §2）。但那几个头会拖出一整个传递闭包，
...
# CherryUSB 也在工程内，编译器得能找到它才能预处理
$Cu = Join-Path $Root 'cherryusb'
if (Test-Path $Cu) {
    $common += @("-I$Cu\core", "-I$Cu\common", "-I$Cu\class\msc",
