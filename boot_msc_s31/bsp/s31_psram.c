...
static int pma_grant_write(uint32_t addr)
{
    ...
    for (i = 0; i < 16u; i++) {
        ...
        kprintf("[psram] PMA%u [%08x,+%uKB) 补上写权限: %08x -> %08x\r\n",
...
static void psram_write_mode_reg(void)
{
    ...
    if ((mr01[1] & 0x1Fu) == AP_VENDOR_ID_AP) {
        kprintf("[psram] 芯片 = AP 系列 (vendor 0x0d)\r\n");
    }
    ...
    kprintf("[psram] mode reg: MR0=%02x MR4=%02x MR8=%02x（期望 %02x/%02x/%02x）\r\n",
...
static int psram_check_connected(void)
{
    ...
    kprintf("[psram] 参考字: 写 %08x 读 %08x -> %s\r\n",
...
static int psram_mmu_map(uint32_t bytes)
{
    ...
    if (!(raw0 & SOC_MMU_PSRAM_VALID) || !(raw0 & SOC_MMU_ACCESS_PSRAM)) {
        kprintf("[psram] MMU 页表项无效: %08x\r\n", (unsigned)raw0);
        return -1;
    }

    kprintf("[psram] MMU: %u 页 @%08x -> PSRAM 物理 0（首项 %08x）\r\n",
...
int s31_psram_init(void)
{
    ...
    kprintf("[psram] === 初始化（16MB 8 线 DDR @200MHz）===\r\n");
    ...
    if (pma_grant_write(S31_PSRAM_VADDR) == 0) {
        kprintf("[psram] 警告: 没找到覆盖 %08x 的 PMA 条目（写 PSRAM 可能会异常）\r\n",
    ...
    if (psram_check_connected() != 0) {
        kprintf("[psram] 芯片没响应 -> 放弃（bootloader 继续跑，只是 App 不能放 PSRAM）\r\n");
        return -1;
    }
     ...
     * 两条路都过 = "App 能搬进 PSRAM 并且读得到"。 */
    {
        ...
        kprintf("[psram] 自检: 原始写+原始读 %s / 原始写+cache读 %s -> %s\r\n",
