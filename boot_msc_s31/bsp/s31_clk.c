 ...
 * 量一下 CPU 主频
 *
 * RISC-V 的 mcycle 每个 CPU 时钟 +1；SYSTIMER 是固定 16 MHz（与 CPU 频率无关）。
 * 两者对表就能算出真实主频 —— **别再靠"rtt 工程说 40MHz"这种二手信息**：
 * USB 那边对 CPU 速度很敏感（我们的 EP0 和 MSC 全在中断里做），
 * 40MHz 还是 320MHz 是完全不同的两件事。
 *
 * 返回 0 = 内核没实现 mcycle（两次读数相同）：那就量不出来，别当 0MHz 用。
 *===========================================================================*/
uint32_t s31_cpu_mhz(void)
{
    uint32_t c0, c1;
    uint32_t guard = 400000000u;
    uint64_t t0, t1, d;

    __asm__ volatile ("csrr %0, mcycle" : "=r"(c0));
    t0 = s31_systimer_ticks();
    do {
        t1 = s31_systimer_ticks();
    } while ((t1 - t0) < 400000u && --guard);   /* 等 25ms（16MHz -> 400000 tick）*/
    ...
    d = t1 - t0;
    return (uint32_t)(((uint64_t)(c1 - c0) * 16u) / d);
}
