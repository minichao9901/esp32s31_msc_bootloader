...
之后**每 5 秒一行 `[app] alive N s`** 心跳（想彻底安静就把 `s31_app/app/main.c` 里的
`HEARTBEAT_MS` 改成 `0`），板载 LED 同时红/绿/蓝轮换 —— 不接串口也能看出板子活着。

> 📌 **复位瞬间会丢日志**：控制台走的是 USB-Serial/JTAG，复位会让它重新枚举，
> 这期间主机的接收会断一下，输出可能丢几行甚至被截断。丢的通常是
> `[boot] probing App...` 那几行或 App 横幅的一部分。
> **别把它当成"跳进 App 就死了"** —— 心跳就是为这件事留的：终端接上以后 5 秒内必有输出。
