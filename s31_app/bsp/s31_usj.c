...
/* 环形缓冲里还有多少字节没送出去（调试打点 / 等排空用）*/
uint32_t s31_usj_tx_pending(void)
{
    return (s_tx_head - s_tx_tail) & USJ_TX_RING_MASK;
}

/* 等发送缓冲排空：带超时，等不到就放弃（没有终端在收时永远排不空）*/
void s31_usj_flush(uint32_t guard)
{
...
/* 有没有主机发来的字节（RX）*/
int s31_usj_getc(void)
{
    if (S31_REG32(S31_USJ_EP1_CONF) & S31_USJ_OUT_EP_DATA_AVAIL) {
        return (int)(S31_REG32(S31_USJ_EP1) & 0xFFu);
    }
    return -1;
}
