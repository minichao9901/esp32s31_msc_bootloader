#===========================================================================
# sync_idf_headers.ps1 —— 把 s31_app 需要的 IDF 头文件"冻结"进工程
#
...
#
# ★ 为什么需要这个脚本
#   `bsp/s31_rmt.c` 直接用 IDF 的 RMT LL 头按**字段名**写寄存器（不手抄位号 ——
#   本项目因为手抄位号栽过两次，见 boot_msc_s31/PSRAM.md §2 §12）。但那几个头会拖出
#   一整个传递闭包，所以把它们拷进 bsp\idf_headers\，工程就不再依赖 IDF 源码树了。
#
# ★ 怎么保证拷全了
...
# ★ 这些头会拖出一整个传递闭包，所以把它们拷进 bsp\idf_headers\，工程就不再依赖 IDF 源码树。
#    本工程用得上的种子只有 RMT 那一条链（bsp/s31_rmt.c）：
#        hal/rmt_ll.h + hal/rmt_types.h + soc/rmt_struct.h + soc/gpio_sig_map.h
#        + soc/clk_tree_defs.h + soc/hp_sys_clkrst_struct.h + soc/hp_system_struct.h
#    不自己解析依赖，靠 `gcc -M` 问编译器（见下面第 3 步）。
#    根系照抄 boot_msc_s31 那份 —— 多几个搜索路径不影响结果，少一个就可能漏头。
$Roots = @(
    (Join-Path $IdfC 'esp_hal_rmt\esp32s31\include'),      # hal/rmt_ll.h
    (Join-Path $IdfC 'esp_hal_rmt\include'),               # hal/rmt_types.h
